...
struct sockaddr_in enterIntoNetwork(int *fd_server, struct sockaddr_in *check_pos_addr) {
    ...
    if (recv_message.msg == CHK_MSG_UP) {
        *check_pos_addr = recv_message.check_pos_addr;
...
void *listBroadcast() {
    ...
    while (1) {
        if ((z = recvfrom(sock_fd, &pck_received, sizeof(pck_received), 0, (struct sockaddr *) &positiveSender, &len)) <
            0) {
...
void *sendBroadcast(void *args) {
    ...
    broadAddr.sin_port = (in_port_t) htons(BROAD_PORT);
