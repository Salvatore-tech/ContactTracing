...
void *signalPositive() {
    ...
    while (1) {
        ...
        if (users_count >= 1) {
            i = rand() % users_count; // Pick a random peer as infected

            if (sendto(sock_fd, &msgToSend, sizeof(msgToSend), 0, (struct sockaddr *) &positivityAddr[i], sizeof(positivityAddr[i])) < 0) {
