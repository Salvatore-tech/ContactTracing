...
void contactNeighbour(int fd_server, const struct sockaddr_in *myAddr) {
    ...
    if (fullRead(fd_server, &msgNumAll, sizeof(msgNumAll)) < 0) { // Get list of active peers from the Server
...
void sendId(const struct sockaddr_in *peerToContact) {
    ...
    if (msgAckNeigh.msg == ACK_CONN) {
        writeNeighbourID(msgAckNeigh.id); // Save the id of the peer contacted
    }
}
...
void *listBroadcast() {
    ...
    while (1) {
        ...
        if (pck_received.msg == MSG_BRD) {
            if (pck_received.id) {
                if (searchNeighbour(pck_received.id, &previousSender, &positiveSender, &toSkip) == 1) {
